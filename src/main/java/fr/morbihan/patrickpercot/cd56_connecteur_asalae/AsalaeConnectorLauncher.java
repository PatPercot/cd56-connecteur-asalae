...
public class AsalaeConnectorLauncher {
	 ...
	 * 
	 * @param args est la ligne de commande
	 * @return le résultat de l'exécution (texte envoyé dans System.out sans le mode verbose)
	 */
	protected String execMain( String[] args )  {
		String output = "";
		this.args = args;
		...
		if (! param.loadFile("param.config")) {
			...
			if (	(args.length == 1 && commandeTester.equals(args[0].toLowerCase()))
				...
				if (commandeTransfert.equals(args[0].toLowerCase())) 
					output = transfert(args[1], args[2]);
			}
		}
		...
		try {
			...
			if (response.statusCode == 200) {
				...
				if (bVerbose)
					...
					output = "ERROR: (tester-serveur) '" + response.message + "'";
					System.out.println(output);
				}
			}
		...
		try {
			...
			if (response.statusCode == 200) {
				...
				if (bVerbose)
					...
					output = "ERROR: (connecter-serveur) '" + response.message + "'";
					System.out.println(output);
				}
			}
	...
	public String transfert(String bordereau, String zip) {
		String output = "";
		AsalaeConnector http = new AsalaeConnector(param);
		...
		try {
			...
			if (retourTransfert.statusCode == 200) {
				...
				if (bVerbose)
					...
					output = "ERROR: (transferer-archive) '" + retourTransfert.message + "'";
					System.out.println(output);
				}
			}
	...
	protected void usage() {
		...
		System.out.println("\t\t--verbose ou -v : mode verbeux");
		System.out.println("\t\t--veryVerbose ou -vv : mode très verbeux");
		System.out.println("");
		System.out.println("\t\t--serveur ou -s : nom du serveur à utiliser (voir param.config)");
		System.out.println("\n");
